...
namespace Frontegg.SDK.Client.Build
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Target(Test,
                DependsOn(Build),
                () => Run("dotnet", $"test ./src/core/Frontegg.SDK.Client.Tests/Frontegg.SDK.Client.Tests.csproj -c Release -r {ArtifactsDir} --no-build -l trx;LogFileName=Frontegg.SDK.Aspney.Tests.xml --verbosity=normal")
                );
            
            Target(
                Pack,
                DependsOn(Build),
                () => Run("dotnet", $"pack ./src/core/Frontegg.SDK.Client/Frontegg.SDK.Client.csproj -c Release -o {ArtifactsDir} --no-build")
                );
